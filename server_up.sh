...
# Про экран заботиться не надо: его поднимает docker-entry.sh, и делает
# это для любой команды. Раньше здесь стоял xvfb-run - и упирался в
# отсутствующий xauth.
if docker compose run --rm avito python3 check_avito.py перфоратор; then
    echo
