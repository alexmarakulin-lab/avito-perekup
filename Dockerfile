# Зеркало Яндекса вместо Docker Hub: сам Hub из России не открывается,
# образ тот же самый. Если зеркало недоступно - меняй эту строку на
# FROM python:3.11-slim или на mirror.gcr.io/library/python:3.11-slim
FROM cr.yandex/mirror/python:3.11-slim

WORKDIR /app

# Откуда брать библиотеки и браузер.
#
# С этого сервера pypi.org отвечает через раз: соединение обрывается на
# середине, как и с github.com. Причина та же - фильтрация на пути, и
# лечится она не настройками, а повторами и запасным зеркалом.
# server_up.sh сам подставит зеркало, если с первого раза не вышло.
ARG PIP_INDEX=https://pypi.org/simple
ARG PW_HOST=

COPY requirements.txt .
# Терпение умеренное: 5 заходов по 20 секунд.
#
# Сперва стояло 20 по 60 - и первая же сборка провисела 22 минуты, честно
# отрабатывая все заходы в закрытый адрес. Долгое ожидание помогает,
# когда связь моргает; когда адрес закрыт насовсем, оно только оттягивает
# переход на зеркало. Упорство здесь снаружи, в server_up.sh: он повторяет
# сборку и со второй попытки идёт в обход.
RUN pip install --no-cache-dir --retries 5 --timeout 20 \
        --index-url "$PIP_INDEX" -r requirements.txt

# Браузер и виртуальный экран.
#
# Зачем экран на сервере, где смотреть некому. Авито пропускает только
# видимый браузер: скрытый режим распознаётся, проверено тремя способами
# (см. avito_browser.py). «Видимый» для браузера означает лишь одно - что
# ему есть куда рисовать. Xvfb даёт экран, которого нет физически: окно
# честно открывается, просто в память, а не на монитор. Для браузера это
# неотличимо от настоящего, и режим у него самый обычный, не скрытый.
#
# --with-deps подтягивает системные библиотеки, без которых Chromium не
# стартует. Их около сотни, руками перечислять - гиблое дело.
#
# PLAYWRIGHT_DOWNLOAD_HOST - на случай, если и до склада браузеров с этого
# сервера не достучаться. Пустая переменная означает «бери откуда обычно».
# Chromium берётся из самого Debian, а не со склада Playwright.
#
# Почему так, а не «как принято». Playwright качает свой Chromium с
# cdn.playwright.dev. С этого сервера туда 184 МБ ехали полтора часа и
# обрывались, а до того один заход завис на четыре часа. Проверено и
# отвергнуто: прямой доступ к складу закрыт, зеркало npmmirror не
# отвечает, через оплаченный прокси канал медленнее 35 КБ/с.
#
# А склад самого Debian с этого сервера работает быстро - сотня системных
# библиотек оттуда встала за секунды. Chromium там есть, и Playwright
# умеет запускать сторонний браузер по прямому пути. Путь прописан ниже в
# AVITO_BROWSER_PATH.
#
# Проверка отдельной строкой оставлена: пакет мог не встать, и лучше
# узнать об этом при сборке, чем через час на сервере.
RUN apt-get update \
    && apt-get install -y --no-install-recommends xvfb chromium \
    && test -x /usr/bin/chromium \
    && apt-get clean && rm -rf /var/lib/apt/lists/*

COPY docker-entry.sh avito_bot.py avito_monitor.py avito_browser.py \
     wb_source.py resale_expert.py check_avito.py env_file.py \
     test_avito_monitor.py test_avito_bot.py test_resale_expert.py ./

# Читаем Авито браузером - так же, как дома.
ENV AVITO_FETCH=browser
# Режим самый обычный, не скрытый: скрытый Авито не пропускает.
ENV AVITO_BROWSER_HEADLESS=0
# Браузер берётся по прямому пути - тот, что поставлен из Debian.
# Дома вместо этого используется установленный Chrome; там переменная
# пустая, и работает выбор по «каналу».
ENV AVITO_BROWSER_PATH=/usr/bin/chromium
ENV AVITO_BROWSER_CHANNEL=""
# Профиль браузера - на том же томе, что и база. Иначе накопленные печенья
# Авито стирались бы при каждой пересборке, и каждый запуск начинался бы с
# капчи: чистый профиль защита не пускает.
ENV AVITO_BROWSER_PROFILE=/data/browser_profile

# База монитора живёт здесь. Том обязателен: без него история цен стирается
# при каждом рестарте, а копится она неделями.
ENV AVITO_DB=/data/avito.db
VOLUME ["/data"]

# Экран поднимает docker-entry.sh - и делает это для любой команды, а не
# только для бота. Стоял бы xvfb-run в CMD, как сначала, - своя команда
# подменяла бы его целиком, и проверка Авито запускалась бы без экрана.
#
# Через sh, а не напрямую: право на запуск у файла может не пережить
# дорогу через Windows и git, а так оно и не нужно.
ENTRYPOINT ["sh", "/app/docker-entry.sh"]
CMD ["python3", "avito_bot.py"]
