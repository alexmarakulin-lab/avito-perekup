...
# Сон компьютера - беда, которая однажды уже стоила суток работы: машина
# уснула в 20:39, утром сторож связи увидел двенадцать часов молчания и
# вышел. Автозапуск этого не лечит, присмотр тоже: пока компьютер спит,
# никто ничего не ищет. Предупреждение показывается при каждом запуске,
# пока настройка не исправлена, - и это нарочно.
try:
    import avito_monitor
    _sleeps, _note = avito_monitor.sleep_setting()
    if _sleeps is False:
        print("\n" + RULER)
        print("  ВНИМАНИЕ: компьютер засыпает сам - " + _note)
        print("")
        print("  Пока он спит, бот не ищет: ни находок, ни канала.")
        print("  Параметры - Система - Питание - «При питании от сети")
        print("  переводить в спящий режим» - Никогда.")
        print(RULER)
except Exception:
    pass          # проверка необязательная, из-за неё запуск падать не должен
