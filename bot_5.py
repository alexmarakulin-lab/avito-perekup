...
# ВАЖНО: gemma первая — у неё отдельный лимит токенов на free tier!
GROQ_MODELS = [
    "gemma2-9b-it",             # быстрая, отдельный лимит — первая для free tier
    "llama-3.3-70b-versatile",  # умная, но тяжёлая
    "llama-3.1-70b-versatile",  # резерв
...
SYSTEM_BASE = """Ты — многопрофильный эксперт-ассистент. Отвечаешь конкретно, по делу, без воды.
Формулы ТОЛЬКО обычным текстом, БЕЗ LaTeX, БЕЗ символов $$, $, \\frac.
Язык: русский. Никогда не придумывай номера нормативных документов."""

# Модули — подключаются только при необходимости
MODULES = {
    "fire": """== СЛАБОТОЧНЫЕ СИСТЕМЫ: СПС и СОУЭ ==
СП 484.1311500.2020 + Изм.№1 (2025) — СПС; СП 3.13130.2026 (с 01.06.2026) / СП 3.13130.2009 (до 01.06.2026) — СОУЭ.
...
СКУД: ГОСТ Р 51241-2008, ГОСТ Р 58485-2024, РД 78.36.003-2002. Идентификаторы: Mifare, Em-Marine, HID, биометрия (ФЗ №152). Контроллеры: Parsec, Bolid, Hikvision, ZKTeco.
...
Документация: ГОСТ Р 21.101-2026, ГОСТ 21.110-2013, Постановление №87 от 16.02.2008, Постановление №486 от 12.04.2025.""",
...
Выдержка: дубовая щепа Medium toast 2-5 г/л, 2-4 недели.""",
    ...
    "recipe_weizen": """== РЕЦЕПТ: Ur-Weizen Clone (Kurpfalz Bräu) ==
...
ХМЕЛЬ: 60 мин Tettnanger 12 г (α6%), 15 мин Tettnanger 7 г.
...
SECTION_TEXTS = {
    ...
    "🌾 Рецепты браг": (
        ...
        "Нажми 🍋 Ur-Weizen — мой рецепт клона Kurpfalz Bräu\n\n"
    ...
    "🍋 Ur-Weizen": (
        ...
        "━━━ ПАРАМЕТРЫ ━━━\n"
        "OG: 1.043–1.045 | FG: ~1.011 | IBU: 11–12 | EBC: ~8 | CO₂: 3,2–3,8 об.\n\n"
        "━━━ ЗАСЫПЬ (4,30 кг) ━━━\n"
